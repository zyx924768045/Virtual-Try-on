...
class MedicalNerModel(object):
    ...
    def __init__(self):
        ...
        self.word2id,_,self.id2tag = pickle.load(
                open(r"F:\自然语言处理课程\领域词性标注\bilstm_crf\checkpoint/word_tag_id.pkl","rb")
            )
        self.model = BiLstmCrfModel(80,2410,200,128,59).build()
        self.model.load_weights(r'F:\自然语言处理课程\领域词性标注\bilstm_crf\checkpoint/best_bilstm_crf_model.h5')

        self.nbd = NerBaseDict(r'F:\自然语言处理课程\领域词性标注\bilstm_crf\checkpoint/diseases.json')
...
if __name__ == '__main__':
    ...
    def index():
        return flask.render_template("/cixin.html")
    ...
    # @app.route('/')
    # def index():
    #     return render_template("cixin.html")
    # app.run()
